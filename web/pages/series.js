...
(function () {
  ...
  function stockcarPlayoffCutline(seriesKey, standings) {
    if (window.TGA && typeof window.TGA.stockcarPlayoffCutline === 'function') {
      return window.TGA.stockcarPlayoffCutline(seriesKey, standings);
    }
    return 0;
  }

  function makeTableSortable() { return P.makeTableSortable.apply(null, arguments); }
  function makeSimpleTableSortable() { return P.makeSimpleTableSortable.apply(null, arguments); }
  ...
  function teamLink(name) {
    ...
    var href = (window.TGA && window.TGA.teamHref && window.TGA.teamHref(raw)) ||
      ('/team/' + encodeURIComponent(slugify(raw)));
...
var F1_DRIVER_CHAMPIONS = {
  ...
  '1997': 'Jacques Villeneuve', '1998': 'Mika Hakkinen', '1999': 'Mika Hakkinen', '2000': 'Michael Schumacher',
  '2001': 'Michael Schumacher', '2002': 'Michael Schumacher', '2003': 'Michael Schumacher', '2004': 'Michael Schumacher',
  '2005': 'Fernando Alonso', '2006': 'Fernando Alonso', '2007': 'Kimi Raikkonen', '2008': 'Lewis Hamilton',
...
/** Display "All" rounds as "1–N" for /season/f1-20xx teams tables. */
function f1SeasonAllRoundsLabel(seriesKeyTeams) {
  var m = String(seriesKeyTeams || '').toLowerCase().match(/^f1-(\d{4})$/);
  if (!m) return '';
  var n = F1_RACES_PER_SEASON[m[1]];
  return n ? ('1–' + n) : '';
}
...
function renderDetail(seriesId, subPath) {
  ...
  // Classes-tab series: URL may be /specs; tab path is /classes
  if (['imsa', 'wec', 'elms', 'gtwce_end', 'gtwce_sprint', 'super_gt', 'dtm'].indexOf((seriesId || '').toLowerCase()) >= 0 && subPath === 'specs') {
    subPath = 'classes';
  }
  ...
  var specsPanelTitleEl = document.getElementById('specs-panel-title');
  if (specsPanelTitleEl) {
    specsPanelTitleEl.textContent = '';
    specsPanelTitleEl.classList.add('hidden');
  }
  ...
  if (sameSeries && !scheduleEmptyNeedReload && subPath !== 'stats') {
    var skSame = (seriesId || '').toLowerCase();
    var isClassesSeriesSame = ['imsa', 'wec', 'elms', 'gtwce_end', 'gtwce_sprint', 'super_gt', 'dtm'].indexOf(skSame) >= 0;
    if (isClassesSeriesSame && subPath === 'specs') subPath = 'classes';
    var specsPathSame = isClassesSeriesSame ? 'classes' : 'specs';
    var navIdx = 0;
    ...
    // Classes-tab series: show static class definitions vs regular Car Specs.
    if (isClassesSeriesSame) {
    ...
    if (!isClassesSeriesSame && subPath === 'specs' && typeof renderF1StaticSpecsIfNeeded === 'function') {
  ...
  var seriesKeyForSpecsTab = (seriesId || '').toLowerCase();
  var isClassesSeries = ['imsa', 'wec', 'elms', 'gtwce_end', 'gtwce_sprint', 'super_gt', 'dtm'].indexOf(seriesKeyForSpecsTab) >= 0;
  if (isClassesSeries && subPath === 'specs') {
    subPath = 'classes';
    if (typeof history !== 'undefined' && history.replaceState) {
      try { history.replaceState(null, '', (base || ('/series/' + seriesIdLower)) + '/classes'); } catch (e) { /* ignore */ }
    }
  }
  var navPages = [
  ...
  if (isClassesSeries) {
    navPages.push({ path: 'classes', labelKey: 'nav.classes' });
  } else {
    navPages.push({ path: 'specs', labelKey: 'nav.carspecs' });
  }
  if (hasStats) {
  ...
  // Classes tab: static class definitions.
  if (isClassesSeries) {
  ...
  if (specsPanel) specsPanel.classList.toggle('hidden', subPath !== (isClassesSeries ? 'classes' : 'specs'));
  if (statsPanel) statsPanel.classList.toggle('hidden', subPath !== 'stats');
  if (historyPanel) historyPanel.classList.toggle('hidden', subPath !== 'history');
    ...
    .then(function (data) {
      ...
      if (seriesKeyTeams !== 'imsa') {
        ...
        if (wrapReset && !(tableReset && wrapReset.contains(tableReset))) {
          // Neutral default — series-specific renderers replace columns (no stock-car Crew Chief).
          wrapReset.innerHTML = '<table class="data-table" id="teams-table"><thead><tr><th>#</th><th data-i18n="th.manufacturer">Manufacturer</th><th data-i18n="th.team">Team</th><th data-i18n="th.no">No.</th><th data-i18n="th.driver">Driver</th></tr></thead><tbody></tbody></table>';
        }
      }

      // F1 season pages: inject year tech regs into teams payload (same mechanism for 2024/2025/2026).
      if (seriesKeyTeams === 'f1' || seriesKeyTeams.indexOf('f1-') === 0) {
        var teamsSpecsYear = (seriesKeyTeams.match(/^f1-(\d{4})$/) || [])[1] || '2026';
        var teamsSpecRows = window['F1_' + teamsSpecsYear + '_TECH_SPEC'] || null;
        if (teamsSpecRows) {
          data.technical_spec = teamsSpecRows;
          hasSpec = true;
        }
      }
      ...
      // For series without Car Specs / Classes content show empty message.
      var hasClassesSpec = seriesKeyTeams === 'imsa'
        ? !!window.IMSA_CLASSES_SPEC
        : !!(window.SERIES_CLASSES_SPEC && window.SERIES_CLASSES_SPEC[seriesKeyTeams]);
          if (!hasSpec && !hasClassesSpec) {
        if (specsEmpty) specsEmpty.classList.remove('hidden');
      }

      // IndyCar Car Specs come from data/teams/indycar.json via /teams (no hardcode).
      function paintTechnicalSpecFromTeamsData(specData) {
        var carWrapPaint = document.getElementById('car-spec-wrap');
        var carModelsWrapPaint = document.getElementById('car-models-table-wrap');
        var carModelsTitlePaint = carWrapPaint && carWrapPaint.querySelector('h4[data-i18n="specs.car_models"]');
        var techSpecWrapPaint = document.getElementById('technical-spec-table-wrap');
        if (!carWrapPaint || !techSpecWrapPaint || !specData || !specData.technical_spec || !specData.technical_spec.length) {
          return false;
        }
        carWrapPaint.classList.remove('hidden');
        if (carModelsTitlePaint) carModelsTitlePaint.classList.add('hidden');
        if (carModelsWrapPaint) carModelsWrapPaint.innerHTML = '';
        if (specsEmpty) specsEmpty.classList.add('hidden');
        var paintRows = specData.technical_spec.filter(function (s) {
          return s && s.key != null && s.value != null && String(s.key).trim() !== '';
        });
        techSpecWrapPaint.className = 'table-wrap';
        techSpecWrapPaint.innerHTML =
          '<table class="data-table table-field-value"><thead><tr><th>' + t('th.field') + '</th><th>' + t('th.value') + '</th></tr></thead><tbody>' +
          paintRows.map(function (s) {
      ...
      function teamRow(tm, i) {
        return '<tr><td class="col-num">' + (i + 1) + '</td><td>' + esc(dash(tm.manufacturer)) + '</td><td>' + teamLink(tm.team) + '</td><td>' + esc(dash(tm.number)) + '</td><td>' + driverLink(tm.driver) + '</td></tr>';
      }
      ...
      function buildF1SeasonTeamsTableHTML(teamsArr, seriesKeyTeams) {
        ...
        while (i < teamsArr.length) {
          ...
          for (var k = 0; k < span; k++) {
            ...
            if (roundsRaw.toLowerCase() === 'all') {
              var allLabel = f1SeasonAllRoundsLabel(seriesKeyTeams);
              if (allLabel) roundsDisplay = allLabel;
            }
      ...
      function buildEntryListTeamsTableHTML(teamsArr, seriesKeyTeams) {
        ...
        while (i < teamsArr.length) {
          ...
          for (var k = 0; k < span; k++) {
            ...
            var roundsRaw = String(tm.rounds || '').trim();
            var roundsDisplay = roundsRaw;
            // F1 season pages: show 1–N instead of All (N from F1_RACES_PER_SEASON).
            if (roundsRaw.toLowerCase() === 'all') {
              var allLabelEntry = f1SeasonAllRoundsLabel(seriesKeyTeams);
              if (allLabelEntry) roundsDisplay = allLabelEntry;
            }
      ...
      // Stock cars: merge Manufacturer / Team / No. / Crew Chief (rowspan).
      // Single <tbody> so manufacturer rowspan can span consecutive teams of the same make.
      // Team striping via tr.group-odd / tr.group-even (not separate tbodies).
      function buildStockCarTeamsBody(teamsArr) {
        ...
        for (var i = 0; i < teamsArr.length; i++) {
          ...
          teamRowSpan[i] = 0;
          numberRowSpan[i] = 0;
          teamGroupClass[i] = 'group-odd';
        }
        for (var i = 0; i < teamsArr.length; i++) {
          if (manRowSpan[i] === -1) continue;
          var manVal = String(teamsArr[i].manufacturer || '').trim();
          var manTeamVal = String(teamsArr[i].team || '').trim();
          var spanMan = 1;
          for (var j = i + 1; j < teamsArr.length; j++) {
            if (String(teamsArr[j].manufacturer || '').trim() !== manVal) break;
            if (String(teamsArr[j].team || '').trim() !== manTeamVal) break;
            spanMan++;
            manRowSpan[j] = -1;
          }
          manRowSpan[i] = spanMan;
        }
        var groupIndex = 0;
        for (var i = 0; i < teamsArr.length; i++) {
          ...
          var gClass = groupIndex % 2 === 0 ? 'group-odd' : 'group-even';
          for (var k = i; k < i + spanTeam; k++) teamGroupClass[k] = gClass;
          groupIndex++;
        }
        for (var i = 0; i < teamsArr.length; i++) {
        ...
        for (var i = 0; i < teamsArr.length; i++) {
          var tm = teamsArr[i];
          var manCell = manRowSpan[i] === -1 ? '' : (manRowSpan[i] > 0 ? '<td rowspan="' + manRowSpan[i] + '" class="stockcar-manufacturer-cell">' + esc(dash(tm.manufacturer)) + '</td>' : '');
          ...
          rows.push('<tr class="' + teamGroupClass[i] + '"><td class="col-num">' + (i + 1) + '</td>' + manCell + teamCell + numberCell + '<td>' + driverLink(tm.driver) + '</td>' + roundsCell + crewChiefCell + '</tr>');
        }
      ...
      if (seriesKeyTeams === 'indycar' && teams.length > 0) {
        ...
        // Nested groups: contiguous same team, then same engine within team only
        var indyRows = [];
        for (var ti = 0; ti < teams.length;) {
          var teamName = String(teams[ti].team || '');
          var teamSpan = 1;
          while (ti + teamSpan < teams.length &&
                 String(teams[ti + teamSpan].team || '') === teamName) {
            teamSpan++;
          }
          var teamFirst = true;
          for (var ei = ti; ei < ti + teamSpan;) {
            var engine = String(teams[ei].manufacturer || '');
            var engSpan = 1;
            while (ei + engSpan < ti + teamSpan &&
                   String(teams[ei + engSpan].manufacturer || '') === engine) {
              engSpan++;
            }
            for (var ri = 0; ri < engSpan; ri++) {
              var tm = teams[ei + ri];
              var cells = '';
              if (teamFirst) {
                cells += '<td rowspan="' + teamSpan + '" class="team-cell">' + teamLink(teamName) + '</td>';
                teamFirst = false;
              }
              if (ri === 0) {
                cells += '<td rowspan="' + engSpan + '" class="manufacturer-cell">' + esc(engine) + '</td>';
              }
              cells +=
        ...
        paintTechnicalSpecFromTeamsData(data);
        return;
      }
      ...
      if (seriesKeyTeams === 'super_formula' && teams.length > 0) {
        ...
        for (var sfi = 0; sfi < sfTeams.length;) {
          ...
          var sfTeamSpan = 1;
          while (sfi + sfTeamSpan < sfTeams.length &&
                 String(sfTeams[sfi + sfTeamSpan].team || '') === sfTeam) {
            sfTeamSpan++;
          }
          var sfTeamFirst = true;
          for (var sfei = sfi; sfei < sfi + sfTeamSpan;) {
            var sfEngine = String(sfTeams[sfei].manufacturer || '');
            var sfEngSpan = 1;
            while (sfei + sfEngSpan < sfi + sfTeamSpan &&
                   String(sfTeams[sfei + sfEngSpan].manufacturer || '') === sfEngine) {
              sfEngSpan++;
            }
            for (var sfj = 0; sfj < sfEngSpan; sfj++) {
              var sft = sfTeams[sfei + sfj];
              var row = '<tr><td class="col-num">' + esc(dash(sft.number)) + '</td>';
              if (sfj === 0) {
                row += '<td rowspan="' + sfEngSpan + '" class="manufacturer-cell">' + esc(dash(sfEngine)) + '</td>';
              }
              if (sfTeamFirst) {
                row += '<td rowspan="' + sfTeamSpan + '" class="team-cell">' + teamLink(sfTeam) + '</td>';
                sfTeamFirst = false;
              }
        ...
        // Early return skips shared Car Specs block — paint from data/teams/super_formula.json.
        paintTechnicalSpecFromTeamsData(data);
        return;
      }
      ...
      if (seriesKeyTeams === 'dtm' && teams.length > 0) {
        ...
        function buildDtmTeamsBody(teamsArr) {
          var rows = [];
          for (var i = 0; i < teamsArr.length;) {
            var teamName = String((teamsArr[i] || {}).team || '');
            var teamSpan = 1;
            while (i + teamSpan < teamsArr.length &&
                   String((teamsArr[i + teamSpan] || {}).team || '') === teamName) {
              teamSpan++;
            }
            var teamFirst = true;
            for (var ci = i; ci < i + teamSpan;) {
              var base = teamsArr[ci] || {};
              var carName = dtmCarValue(base);
              var engineName = String(base.power_unit || '');
              var carSpan = 1;
              while (ci + carSpan < i + teamSpan) {
                var nextCar = teamsArr[ci + carSpan] || {};
                if (dtmCarValue(nextCar) !== carName || String(nextCar.power_unit || '') !== engineName) break;
                carSpan++;
              }
              for (var j = 0; j < carSpan; j++) {
                var tm = teamsArr[ci + j] || {};
                var row = '<tr>';
                if (teamFirst) {
                  row += '<td rowspan="' + teamSpan + '">' + teamLink(teamName) + '</td>';
                  teamFirst = false;
                }
                if (j === 0) {
                  row += '<td rowspan="' + carSpan + '">' + esc(dash(carName)) + '</td>';
                  row += '<td rowspan="' + carSpan + '">' + esc(dash(engineName)) + '</td>';
                }
                row += '<td class="col-num">' + esc(dash(tm.number)) + '</td>';
                row += '<td>' + driverLink(tm.driver) + '</td>';
                row += '<td>' + esc(dash(tm.rounds)) + '</td>';
      ...
      // Special Supercars renderer: championship entry list (manufacturer / team / driver / rounds).
      if (seriesKeyTeams === 'supercars') {
        ...
        // Championship entries + weekend substitutes (full_time false, same car number).
        var champTeams = teams.filter(function (t) {
          if (t.full_time === true) return true;
          if (t.full_time === false && String(t.rounds || '').trim() !== '') return true;
          return false;
        });
        ...
        champTeams.sort(function (a, b) {
          ...
          if (a.full_time !== b.full_time) return a.full_time ? -1 : 1;
          return 0;
        });
        ...
        tableWrap.classList.remove('hidden');
        // Car Specs from API (data/teams/supercars.json) — no static JS fallback.
        (function paintSupercarsSpecsFromApi() {
          ...
          if (!carWrap) return;
          if (data.car_models && data.car_models.length > 0) {
            carWrap.classList.remove('hidden');
            if (carModelsTitle) carModelsTitle.classList.remove('hidden');
            if (carModelsWrap) {
              carModelsWrap.innerHTML = '<table class="data-table"><thead><tr><th>' + t('th.manufacturer') + '</th><th>' + t('th.model') + '</th></tr></thead><tbody>' +
                data.car_models.map(function (c) {
                  return '<tr><td>' + esc(dash(c.manufacturer)) + '</td><td>' + esc(dash(c.model)) + '</td></tr>';
                }).join('') + '</tbody></table>';
            }
          }
          if (techSpecWrap && data.technical_spec && data.technical_spec.length > 0) {
            carWrap.classList.remove('hidden');
            if (specsEmpty) specsEmpty.classList.add('hidden');
            techSpecWrap.innerHTML = '<table class="data-table table-field-value"><thead><tr><th>' + t('th.field') + '</th><th>' + t('th.value') + '</th></tr></thead><tbody>' +
              data.technical_spec.map(function (s) {
                return '<tr><td class="col-field">' + esc(dash(localizeSpecKey(s.key))) + '</td><td>' + esc(dash(localizeSpecValue(s.value))) + '</td></tr>';
              }).join('') + '</tbody></table>';
          }
          var scEngines = Array.isArray(data.engines) ? data.engines : [];
          if (scEngines.length > 0 && enginesWrap) {
            enginesWrap.innerHTML = '<table class="data-table"><thead><tr><th>' + t('specs.car_model') + '</th><th>' + t('specs.engine_spec') + '</th></tr></thead><tbody>' +
          ...
          var scHomolog = Array.isArray(data.homologation) ? data.homologation : [];
          if (scHomolog.length > 0 && homologationWrap) {
            homologationWrap.innerHTML = '<table class="data-table"><thead><tr><th>' + t('th.manufacturer') + '</th><th>' + t('specs.homologating_team') + '</th></tr></thead><tbody>' +
      ...
      if (seriesKeyTeams.indexOf('f1-') === 0 && teams.length > 0) {
        // F1 season slugs (f1-2024 etc.): Entrant/Team | Constructor | Chassis | Power unit | No. | Driver | Rounds.
        ['teams-fulltime-wrap', 'teams-fulltime-title', 'teams-parttime-wrap', 'teams-parttime-title',
         'teams-nonchartered-wrap', 'teams-nonchartered-title',
         'teams-endurance-wrap', 'teams-wildcard-wrap'].forEach(function (id) {
          var el = document.getElementById(id);
      ...
      if (gtEnduranceTeamsSeries.indexOf(seriesKeyTeams) >= 0 && teams.length > 0) {
        ...
        function gtClassTableBlock(title, teamList) {
        ...
        function insertGtClassBlocksHtml(blocksHtml) {
          wrapGt.classList.add('hidden');
          wrapGt.innerHTML = '';
          if (dualWrapGt && blocksHtml) {
            var container = document.createElement('div');
            container.className = 'wec-teams-class-blocks gt-teams-class-blocks';
            container.innerHTML = blocksHtml;
            dualWrapGt.insertBefore(container, wrapGt);
          }
        }

        function normalizeGtwceTeamsClassKey(cls) {
          var c = String(cls || '').trim().toUpperCase().replace(/\s+/g, ' ');
          if (c === 'PRO' || c === 'PRO CUP') return 'pro';
          if (c === 'GOLD' || c === 'GOLD CUP') return 'gold';
          if (c === 'SILVER' || c === 'SILVER CUP') return 'silver';
          if (c === 'BRONZE' || c === 'BRONZE CUP') return 'bronze';
          if (c.indexOf('PRO-AM') >= 0 || c === 'PRO AM' || c === 'PRO-AM CUP') return 'pro_am';
          return '';
        }
        ...
        if (seriesKeyTeams === 'wec') {
          ...
          var wecHtml = '';
          if (hypercarTeams.length > 0) wecHtml += gtClassTableBlock(localizeRacingClass('Hypercar'), hypercarTeams);
          if (lmgt3Teams.length > 0) wecHtml += gtClassTableBlock(localizeRacingClass('LMGT3'), lmgt3Teams);
          if (lmp2Teams.length > 0) wecHtml += gtClassTableBlock(localizeRacingClass('LMP2'), lmp2Teams);
          insertGtClassBlocksHtml(wecHtml);
          return;
        }

        // GTWCE Endurance / Sprint: Pro, Gold, Silver, Bronze, Pro-Am — one table per class.
        if (seriesKeyTeams === 'gtwce_end' || seriesKeyTeams === 'gtwce_sprint') {
          var gtwceSections = [
            { key: 'pro', title: t('class.gtwce_pro') },
            { key: 'gold', title: t('class.gtwce_gold') },
            { key: 'silver', title: t('class.gtwce_silver') },
            { key: 'bronze', title: t('class.gtwce_bronze') }
          ];
          if (seriesKeyTeams === 'gtwce_sprint') {
            gtwceSections.push({ key: 'pro_am', title: t('class.gtwce_pro_am') });
          }
          var gtwceHtml = '';
          gtwceSections.forEach(function (sec) {
            var list = teams.filter(function (tm) {
              return normalizeGtwceTeamsClassKey(tm.class) === sec.key;
            });
            if (list.length > 0) gtwceHtml += gtClassTableBlock(sec.title, list);
          });
          var gtwceOther = teams.filter(function (tm) {
            return normalizeGtwceTeamsClassKey(tm.class) === '';
          });
          if (gtwceOther.length > 0) {
            var byRawClass = {};
            gtwceOther.forEach(function (tm) {
              var raw = String(tm.class || '').trim() || '—';
              if (!byRawClass[raw]) byRawClass[raw] = [];
              byRawClass[raw].push(tm);
            });
            Object.keys(byRawClass).sort().forEach(function (raw) {
              gtwceHtml += gtClassTableBlock(localizeRacingClass(raw), byRawClass[raw]);
            });
          }
          insertGtClassBlocksHtml(gtwceHtml);
          return;
        }
      ...
      // FT/PT split tables only for stock-car (Supercars uses custom renderer + rounds).
      // Do not trust API full_time:false zero-values on open-wheel/endurance/IndyCar.
      var usesFtPtSeries = window.TGA && typeof window.TGA.seriesUsesFullTimeFlag === 'function'
        ? window.TGA.seriesUsesFullTimeFlag(seriesKeyTeams)
        : (isStockCarSeriesTeams);
      var hasFullTimeFlag = usesFtPtSeries && teams.some(function (t) {
        return t && (t.full_time === true || t.full_time === false || t.hasOwnProperty('full_time'));
      });
      ...
      if (isCupWithNonChartered) {
        ...
        if (teamsFulltimeBody && teams.length > 0) {
          if (isStockCarSeriesTeams) {
            ...
            addObjectTableSort(fulltimeWrap.querySelector('.data-table'), teams, teamRow, [null, 'manufacturer', 'team', 'number', 'driver']);
          }
        }
        ...
        if (teamsNoncharteredBody) {
          if (isStockCarSeriesTeams) {
            ...
            addObjectTableSort(noncharteredWrap.querySelector('.data-table'), data.teams_non_chartered, teamNonCharteredRow, [null, 'manufacturer', 'team', 'number', 'driver']);
          }
        }
        ...
        if (teamsFulltimeBody && fullTime.length > 0) {
          if (isStockCarSeriesTeams) {
            ...
            addObjectTableSort(fulltimeWrap.querySelector('.data-table'), fullTime, seriesKeyTeams === 'f1' ? null : teamRow, [null, 'manufacturer', 'team', 'number', 'driver'], seriesKeyTeams === 'f1' ? function (dataCopy) { return buildOpenWheelTeamsBody(dataCopy); } : null);
          }
        }
        if (teamsParttimeBody && partTime.length > 0) {
          if (isStockCarSeriesTeams) {
            ...
            addObjectTableSort(parttimeWrap.querySelector('.data-table'), partTime, seriesKeyTeams === 'f1' ? null : partTimeRow, [null, 'manufacturer', 'team', 'number', 'driver'], seriesKeyTeams === 'f1' ? function (dataCopy) { return buildOpenWheelTeamsBody(dataCopy); } : null);
          }
        }
      } else {
        ...
        if (teams.length > 0) {
          ...
          if (isStockCarSeriesTeams) {
            ...
            addObjectTableSort(document.getElementById('teams-table-wrap').querySelector('.data-table'), teams, teamRow, [null, 'manufacturer', 'team', 'number', 'driver']);
          }
        }
      }
      var carWrap = document.getElementById('car-spec-wrap');
      ...
      // F1: static Car Specs table independent of /teams response (year-aware for /season/f1-20xx).
      var seriesIdLowerForSpecs = (seriesId || '').toLowerCase();
      var isF1SeasonSpecs = seriesIdLowerForSpecs === 'f1' || seriesIdLowerForSpecs.indexOf('f1-') === 0;
      var f1SpecsYear = (seriesIdLowerForSpecs.match(/^f1-(\d{4})$/) || [])[1] || '2026';
      var f1SpecsRowsSource = window['F1_' + f1SpecsYear + '_TECH_SPEC']
        || (window.TGA && window.TGA['F1_' + f1SpecsYear + '_TECH_SPEC'])
        || [];
      if (isF1SeasonSpecs && carWrap && techSpecWrap && f1SpecsRowsSource.length) {
        carWrap.classList.remove('hidden');
        ...
        if (specsPanelStatic) {
          var specsTitleStatic = document.getElementById('specs-panel-title');
          if (specsTitleStatic) {
            var techRegsKey = 'specs.tech_regulations_' + f1SpecsYear;
            var techRegsLabel = t(techRegsKey);
            specsTitleStatic.textContent = (techRegsLabel && techRegsLabel !== techRegsKey)
              ? techRegsLabel
              : ('Technical regulations ' + f1SpecsYear);
            specsTitleStatic.classList.remove('hidden');
          }
        }
        ...
        var f1SpecRows = f1SpecsRowsSource.slice();
        var f1Sections = [];
        var currentTitleF1 = '';
        var currentRowsF1 = [];
      ...
      if (carWrap && data.technical_spec && data.technical_spec.length > 0) {
        ...
        if (techSpecWrap) {
          function specCellVal(s, val) {
            var seriesForPowerNote = (seriesId || '').toLowerCase();
            if (seriesForPowerNote === 'nascar_cup' && s.key && s.key.toLowerCase().trim() === 'power output') {
              // Packages already listed in data/teams/nascar_cup.json (NASCAR 2026 rule updates).
              return esc(dash(val));
            }
            return (val || '').indexOf('\n') >= 0
              ? (val || '').split('\n').map(function (p) { return esc(p); }).join('<br>')
        ...
        // Extra Engines and Homologation tables for Supercars (from teams JSON / API).
        if ((seriesId || '').toLowerCase() === 'supercars') {
          if (enginesWrap) {
            enginesWrap.innerHTML = '';
            enginesWrap.classList.add('hidden');
          ...
          if (enginesTitle) enginesTitle.classList.add('hidden');
          var scEngines = (data && Array.isArray(data.engines)) ? data.engines : [];
          if (scEngines.length > 0 && enginesWrap) {
          ...
          if (homologationTitle) homologationTitle.classList.add('hidden');
          var scHomolog = (data && Array.isArray(data.homologation)) ? data.homologation : [];
          if (scHomolog.length > 0 && homologationWrap) {
      ...
      // Classes-tab series: title + static definitions (imsa-classes-static).
      var currentSeriesSlug = (window.location.pathname.split('/')[2] || '').toLowerCase().replace(/-/g, '_');
      if (['imsa', 'wec', 'elms', 'gtwce_end', 'gtwce_sprint', 'super_gt', 'dtm'].indexOf(currentSeriesSlug) >= 0) {
        var specsTitleEl = document.getElementById('specs-panel-title');
        if (specsTitleEl) {
          specsTitleEl.textContent = '';
          specsTitleEl.classList.add('hidden');
        }
        var legacyImsaWrap = document.getElementById('imsa-classes-wrap');
        if (legacyImsaWrap) legacyImsaWrap.classList.add('hidden');
        if (typeof renderImsaClassesSpecIfNeeded === 'function') renderImsaClassesSpecIfNeeded();
      }
      translateStaticUI();
    })
  ...
  API.getSeriesStandings(seriesId)
    .then(function (data) {
      function renderStandings(dataObj) {
        ...
        var chaseNoteReset = document.getElementById('standings-chase-note');
        if (chaseNoteReset && sk !== 'nascar_cup' && sk !== 'noaps' && sk !== 'nascar_truck') {
          chaseNoteReset.textContent = '';
          chaseNoteReset.classList.add('hidden');
        }
        ...
        var isF1ConstructorStandings = window.TGA && window.TGA.isConstructorStandingsSeries && window.TGA.isConstructorStandingsSeries(sk);
        var f1StandingsMode = isF1ConstructorStandings && window.TGA.getStandingsMode
          ? window.TGA.getStandingsMode(sk)
          : 'driver';
        var showConstructors = isF1ConstructorStandings && f1StandingsMode === 'constructors';
        if (showConstructors) {
          rows = (dataObj && dataObj.teams && Array.isArray(dataObj.teams)) ? dataObj.teams.slice() : [];
        }
        if (!isF1ConstructorStandings && window.TGA && window.TGA.hideStandingsModeNav) {
        ...
        if (isF1ConstructorStandings && window.TGA.renderStandingsModeNav) {
          window.TGA.renderStandingsModeNav(sk, f1StandingsMode, function () {
            renderStandings(dataObj);
          });
        }
        ...
        if (sk === 'f1' || sk === 'f2' || sk === 'f3' || sk === 'frec' || sk === 'f4_it' || String(currentSeriesId || '').toLowerCase().indexOf('f1-') === 0) {
          ...
          if (hasRaceCols && eventNames.length >= raceOrder.length && theadElF1) {
            // F1 season pages (/season/f1-20xx/standings): ·S / ·F suffixes.
        ...
        var includeManufacturer = !showConstructors && (sk !== 'super_formula' && sk !== 'psc');
        var hasCar    = !showConstructors && rows.some(function (r) { return r.car; });
        var hasWth    = !showConstructors && rows.some(function (r) { return r.wth; });
        var hasStatus = !showConstructors && rows.some(function (r) { return r.status; });
        var supportsStages = (sk === 'nascar_cup' || sk === 'noaps' || sk === 'nascar_truck');
        var hasStages = !showConstructors && supportsStages && rows.some(function (r) {
          if (r.stages == null) return false;
          var s = String(r.stages).trim();
          return s !== '' && s !== '0' && s !== '—';
        });
        var hasPlayoffPts = !showConstructors && supportsStages && rows.some(function (r) {
          var p = r && r.playoff_points != null ? String(r.playoff_points).trim() : '';
          return p !== '' && p !== '0' && p !== '—';
        });
        var chaseStart = (dataObj && dataObj.chase && dataObj.chase.regular_season_races)
          ? Number(dataObj.chase.regular_season_races) : 0;
        var chaseNoteEl = document.getElementById('standings-chase-note');
        if (chaseNoteEl) {
          var chaseRound = dataObj && dataObj.chase && dataObj.chase.round;
          var chaseKey = (window.TGA && window.TGA.stockcarChaseRoundLabelKey)
            ? window.TGA.stockcarChaseRoundLabelKey(chaseRound)
            : '';
          var chaseLabel = chaseKey ? t(chaseKey) : '';
          if (supportsStages && chaseLabel && chaseLabel !== chaseKey && dataObj.chase && dataObj.chase.active) {
            chaseNoteEl.textContent = chaseLabel;
            chaseNoteEl.classList.remove('hidden');
          } else {
            chaseNoteEl.textContent = '';
            chaseNoteEl.classList.add('hidden');
          }
        }
        ...
        if (showConstructors) {
          th += '<th>' + t('th.constructor') + '</th>';
        } else {
          th += '<th>' + t('th.driver') + '</th><th>' + t('th.team') + '</th>';
        }
        if (includeManufacturer) th += '<th>' + esc(manufacturerLabel) + '</th>';
        for (var i = 0; i < raceOrder.length; i++) {
          var raceHeadCls = 'col-race' + ((chaseStart > 0 && i === chaseStart) ? ' supercars-stage-divider' : '');
          th += raceHeaderCellHtml(i, esc(raceHeaderLabel(raceOrder[i], i)), raceHeadCls);
        }
        if (hasStages) th += '<th>' + t('th.stage_col') + '</th>';
        if (hasPlayoffPts) th += '<th>' + t('th.playoff_pts') + '</th>';
        ...
        if (theadEl && sk === 'supercars' && raceOrder.length > 0) {
          ...
          function supercarsRaceNum(code) {
            var m = String(code || '').match(/^[A-Z]+(\d+)$/i);
            return (m && m[1]) ? m[1] : '';
          }
          var scGroups = [];
          for (var g = 0; g < raceOrder.length; g++) {
            var gp = supercarsRacePrefix(raceOrder[g]);
            if (scGroups.length && scGroups[scGroups.length - 1].prefix === gp) {
              scGroups[scGroups.length - 1].count++;
            } else {
              scGroups.push({ prefix: gp, count: 1, start: g });
            }
          }
          var topRowSc = '<tr class="standings-header-row-top">';
          topRowSc += '<th class="col-num" rowspan="2">' + t('th.pos') + '</th>';
          if (hasCar) topRowSc += '<th class="col-car" rowspan="2">' + t('th.no') + '</th>';
          topRowSc += '<th rowspan="2">' + t('th.driver') + '</th>';
          topRowSc += '<th rowspan="2">' + t('th.team') + '</th>';
          topRowSc += '<th rowspan="2">' + esc(manufacturerLabel) + '</th>';
          for (var gti = 0; gti < scGroups.length; gti++) {
          ...
          if (hasStatus) topRowSc += '<th rowspan="2">' + t('th.status') + '</th>';
          topRowSc += '<th class="col-pts" rowspan="2">' + t('th.pts') + '</th></tr>';

          var bottomRowSc = '<tr id="standings-thead">';
          var prevPrefix = '';
      ...
      function renderStandingsRows(list) {
        var playoffCutline = stockcarPlayoffCutline(sk, dataObj);
        standingsBody.innerHTML = list.map(function (row) {
          ...
          var rowClass = (window.TGA && window.TGA.stockcarPlayoffRowClass)
            ? window.TGA.stockcarPlayoffRowClass(sk, posNum, row, dataObj)
          ...
          for (var j = 0; j < raceOrder.length; j++) {
            ...
            if (hasPlayoffPts) td += '<td>' + esc(dash(row.playoff_points)) + '</td>';
      ...
      var stThs = standingsHeaderCells(theadEl, theadRow);
        ...
        var playoffOff = hasPlayoffPts ? 1 : 0;
        var wthOff = hasWth ? 1 : 0;
        var statusOff = hasStatus ? 1 : 0;
        var manuOff = includeManufacturer ? 1 : 0;
        var nameCols = showConstructors ? 1 : 2; // constructor only vs driver+team
        var baseAfterRaces = 1 + carOff + nameCols + manuOff + raceOrder.length;
        var ptsColIndex = baseAfterRaces + stageOff + playoffOff + wthOff + statusOff;
        function getStandingVal(row, colIndex) {
          var raceIdx = colIndex - (1 + carOff + nameCols + manuOff);
          if (colIndex === 0)                               return row.pos || 0;
          if (hasCar && colIndex === 1)                     return row.car || '';
          if (colIndex === 1 + carOff)                      return row.driver || '';
          if (!showConstructors && colIndex === 2 + carOff) return row.team || '';
          if (includeManufacturer && colIndex === 1 + carOff + nameCols) return row.manufacturer || '';
          ...
          if (hasPlayoffPts && colIndex === baseAfterRaces + stageOff) return row.playoff_points || '';
          if (hasWth && colIndex === baseAfterRaces + stageOff + playoffOff) return row.wth || '';
          if (hasStatus && colIndex === baseAfterRaces + stageOff + playoffOff + wthOff) return row.status || '';
  ...
  if (statsPanel && hasStats && subPath === 'stats') {
    ...
    function splitStatsSlashParts(s) {
      return String(s || '').split(/\s*\/\s*/).map(function (p) { return p.trim(); }).filter(function (p) { return p && p !== '—'; });
    }

    function sortStatsCarNumbers(cars) {
    ...
    function statsRowHasValue(list, key) {
    ...
    function sortStatsDataRows(rows, key, dir, numericKeys) {
    ...
    function statsRowHasPositive(list, key) {
      return list.some(function (row) {
    ...
    function statsRowNameKey(row) {
      if (row && row.driver) return 'driver';
      if (row && row.team) return 'team';
      if (row && row.manufacturer) return 'manufacturer';
      return 'driver';
    }

    function compareStatsValueDesc(va, vb, key, numericKeys) {
      var ae = (va === null || va === undefined || va === '');
      var be = (vb === null || vb === undefined || vb === '');
      if (ae && be) return 0;
      if (ae) return 1;
    ...
    function compareStatsRowsDesc(a, b, keys, numericKeys, nameKey) {
      for (var i = 0; i < keys.length; i++) {
        var c = compareStatsValueDesc(a[keys[i]], b[keys[i]], keys[i], numericKeys);
        if (c !== 0) return c;
      }
      return String(a[nameKey] || '').localeCompare(String(b[nameKey] || ''), undefined, { numeric: true });
    }

    function normalizeStatsSeriesKey(seriesKey) {
      var sk = String(seriesKey || '').toLowerCase();
      if (sk.indexOf('f1-') === 0) return 'f1';
      return sk;
    }

    function defaultStatsSortSpec(seriesKey, rows) {
      var sk = normalizeStatsSeriesKey(seriesKey);
      var STOCK = ['nascar_cup', 'noaps', 'nascar_truck', 'arca', 'nascar_modified'];
      if (STOCK.indexOf(sk) >= 0) {
        return { keys: ['points', 'wins', 'top5', 'top10'], primary: 'points' };
      }
      if (sk === 'f1' || sk === 'f2' || sk === 'f3') {
        return { keys: ['wins', 'podiums', 'top5', 'top10'], primary: 'wins' };
      }
      if (sk === 'supercars') {
        return { keys: ['wins', 'top5', 'top10'], primary: 'wins' };
      }
      if (sk === 'psc') {
        return { keys: ['points', 'wins', 'podiums', 'top5', 'top10'], primary: 'points' };
      }
      if (statsRowHasPositive(rows, 'points')) {
        return { keys: ['points', 'wins', 'podiums', 'top5', 'top10'], primary: 'points' };
      }
      if (statsRowHasValue(rows, 'wins')) {
        return { keys: ['wins', 'podiums', 'top5', 'top10'], primary: 'wins' };
      }
      if (statsRowHasValue(rows, 'podiums')) {
        return { keys: ['podiums', 'top5', 'top10'], primary: 'podiums' };
      }
      return { keys: ['races'], primary: 'races' };
    }

    function defaultStatsSortKey(seriesKey, rows) {
      return defaultStatsSortSpec(seriesKey, rows).primary;
    }

    function applyDefaultStatsSort(rows, seriesKey, numericKeys, nameKey) {
      if (!rows || !rows.length) return null;
      var spec = defaultStatsSortSpec(seriesKey, rows);
      var nk = nameKey || statsRowNameKey(rows[0]);
      rows.sort(function (a, b) {
        return compareStatsRowsDesc(a, b, spec.keys, numericKeys, nk);
      });
      return spec.primary;
    }

    function markStatsSortHeader(table, columns, key, dir) {
      var headRow = table && table.querySelector('thead tr');
      if (!headRow || !key) return;
      var ths = headRow.querySelectorAll('th');
      for (var c = 0; c < columns.length; c++) {
        if (columns[c].key !== key) continue;
        ths[c].classList.add(dir < 0 ? 'sort-desc' : 'sort-asc');
        ths[c].dataset.sortDir = dir < 0 ? 'desc' : 'asc';
        break;
      }
    }

    function markStatsSortHeaderByIndex(table, colIndex, dir) {
      var headRow = table && table.querySelector('thead tr');
      if (!headRow || colIndex < 0) return;
      var ths = headRow.querySelectorAll('th');
      if (colIndex >= ths.length) return;
      ths[colIndex].classList.add(dir < 0 ? 'sort-desc' : 'sort-asc');
      ths[colIndex].dataset.sortDir = dir < 0 ? 'desc' : 'asc';
    }

    function f1DriverStatsSortColumnIndex(primaryKey) {
      var map = { races: 4, wins: 5, top2: 6, top3: 7, podiums: 8, top5: 9, top10: 10 };
      return map[primaryKey] != null ? map[primaryKey] : 5;
    }

    function f1ManufacturerStatsSortColumnIndex(primaryKey) {
      var map = { races: 2, wins: 3, top2: 4, top3: 5, podiums: 6, top5: 7, top10: 8 };
      return map[primaryKey] != null ? map[primaryKey] : 3;
    }

    function formatStatsSlashStack(parts, renderPart) {
      if (!parts || parts.length === 0) return '—';
      if (parts.length === 1) return renderPart(parts[0]);
      return '<span class="stats-stint-stack">' + parts.map(function (p) {
        return '<span class="stats-stint-line">' + renderPart(p) + '</span>';
      }).join('') + '</span>';
    }

    function formatStatsCarCell(carRaw, teamRaw, escFn) {
      var escPart = escFn || esc;
      var cars = sortStatsCarNumbers(splitStatsSlashParts(carRaw));
      var teams = splitStatsSlashParts(teamRaw);
      if (cars.length === 0) return '—';
      // Same team, multiple numbers (e.g. #55 and #35 for Joey Gase Motorsports) — one line in #.
      if (cars.length > 1 && teams.length <= 1) {
        return cars.map(function (c) { return escPart(c); }).join('<span class="stats-slash"> / </span>');
      }
      return formatStatsSlashStack(cars, function (c) { return escPart(c); });
    }

    function formatStatsTeamCell(teamRaw, carRaw) {
      if (!teamRaw || String(teamRaw).trim() === '' || String(teamRaw).trim() === '—') return '—';
      var teams = splitStatsSlashParts(teamRaw);
      var cars = splitStatsSlashParts(carRaw);
      if (teams.length === 0) return '—';
      if (teams.length === 1) return teamLink(teams[0]);
      if (cars.length > 1 && cars.length === teams.length) {
        return formatStatsSlashStack(teams, teamLink);
      }
      return formatStatsSlashStack(teams, teamLink);
    }
    ...
    if (isF1LikeStats) {
      ...
      API.getSeriesStats(seriesId)
        .then(function (data) {
          ...
          var f1StatsNumericKeys = [
            'races', 'wins', 'top2', 'top3', 'podiums', 'top5', 'top10', 'poles',
            'avg_start', 'avg_qualifying', 'avg_finish', 'q2_passes', 'q3_passes', 'laps_led', 'laps_completed', 'dnfs'
          ];
          applyDefaultStatsSort(rows, sidLowerForStats, f1StatsNumericKeys, 'driver');

          thead.innerHTML =
            '<th class="col-stat-num col-num">' + t('th.pos') + '</th>' +
            '<th class="col-stat-num col-num">' + t('th.no') + '</th>' +
            '<th class="col-stat-text">' + t('th.driver') + '</th>' +
            '<th class="col-stat-text">' + t('th.team') + '</th>' +
            '<th class="col-stat-num">' + t('stats.starts') + '</th>' +
            '<th class="col-stat-num">' + t('standings.wins') + '</th>' +
            '<th class="col-stat-num">' + t('stats.top2') + '</th>' +
            '<th class="col-stat-num">' + t('stats.top3') + '</th>' +
            '<th class="col-stat-num">' + t('stats.podiums') + '</th>' +
            '<th class="col-stat-num">' + t('standings.top5') + '</th>' +
            '<th class="col-stat-num">' + t('standings.top10') + '</th>' +
            '<th class="col-stat-num">' + t('standings.avg_start') + '</th>' +
            '<th class="col-stat-num">' + t('stats.avg_qualifying') + '</th>' +
            '<th class="col-stat-num">' + t('standings.poles') + '</th>' +
            '<th class="col-stat-num">' + t('standings.avg_finish') + '</th>' +
            '<th class="col-stat-num">' + t('stats.q2') + '</th>' +
            '<th class="col-stat-num">' + t('stats.q3') + '</th>' +
            '<th class="col-stat-num">' + t('standings.laps_led') + '</th>' +
            '<th class="col-stat-num">' + t('stats.laps_completed') + '</th>' +
            '<th class="col-stat-num">DNF</th>';
          ...
          var html = rows.map(function (row, idx) {
            ...
            return '' +
              '<tr>' +
                '<td class="col-stat-num col-num">' + pos + '</td>' +
                '<td class="col-stat-num col-car">' + formatStatsCarCell(car, team, escHtmlF1) + '</td>' +
                '<td class="col-stat-text">' + driverLink(driver) + '</td>' +
                '<td class="col-stat-text">' + formatStatsTeamCell(team, car) + '</td>' +
                '<td class="col-stat-num">' + races + '</td>' +
                '<td class="col-stat-num">' + wins + '</td>' +
                '<td class="col-stat-num">' + top2 + '</td>' +
                '<td class="col-stat-num">' + top3 + '</td>' +
                '<td class="col-stat-num">' + podiums + '</td>' +
                '<td class="col-stat-num">' + top5 + '</td>' +
                '<td class="col-stat-num">' + top10 + '</td>' +
                '<td class="col-stat-num">' + avgStart + '</td>' +
                '<td class="col-stat-num">' + avgQual + '</td>' +
                '<td class="col-stat-num">' + poles + '</td>' +
                '<td class="col-stat-num">' + avgFinish + '</td>' +
                '<td class="col-stat-num">' + q2 + '</td>' +
                '<td class="col-stat-num">' + q3 + '</td>' +
                '<td class="col-stat-num">' + lapsLed + '</td>' +
                '<td class="col-stat-num">' + lapsCompleted + '</td>' +
                '<td class="col-stat-num">' + dnfs + '</td>' +
              '</tr>';
          }).join('');
          ...
          if (typeof makeSimpleTableSortable === 'function') {
            makeSimpleTableSortable(table, { renumberFirstColumn: true });
          }
          markStatsSortHeaderByIndex(table, f1DriverStatsSortColumnIndex(defaultStatsSortKey(sidLowerForStats, rows)), -1);
          ...
          if (manTable && manRows.length > 0) {
            ...
            if (manTbody) {
              ...
              applyDefaultStatsSort(manRows, sidLowerForStats, f1StatsNumericKeys, 'manufacturer');
              manTbody.innerHTML = manRows.map(function (row, idx) {
                ...
                return '<tr>' +
                  '<td class="col-stat-num col-num">' + (idx + 1) + '</td>' +
                  '<td class="col-stat-text">' + escHtmlF1(row.manufacturer || '') + '</td>' +
                  '<td class="col-stat-num">' + (row.races || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.wins || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.top2 || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.top3 || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.podiums != null ? row.podiums : (row.wins || 0) + (row.top2 || 0) + (row.top3 || 0)) + '</td>' +
                  '<td class="col-stat-num">' + (row.top5 || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.top10 || 0) + '</td>' +
                  '<td class="col-stat-num">' + avgStart + '</td>' +
                  '<td class="col-stat-num">' + avgQual + '</td>' +
                  '<td class="col-stat-num">' + avgFinish + '</td>' +
                  '<td class="col-stat-num">' + (row.q2_passes != null ? row.q2_passes : 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.q3_passes != null ? row.q3_passes : 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.laps_led || 0) + '</td>' +
                  '<td class="col-stat-num">' + (row.laps_completed != null ? row.laps_completed : '—') + '</td>' +
                  '<td class="col-stat-num">' + (row.dnfs || 0) + '</td>' +
                  '</tr>';
              }).join('');
              // Enable simple column sorting for F1 manufacturer stats.
              if (typeof makeSimpleTableSortable === 'function') {
                makeSimpleTableSortable(manTable, { renumberFirstColumn: true });
              }
              markStatsSortHeaderByIndex(manTable, f1ManufacturerStatsSortColumnIndex(defaultStatsSortKey(sidLowerForStats, manRows)), -1);
            }
          } else {
    ...
    function loadStats(attempt) {
      ...
      .then(function (data) {
        ...
        var STATS_TEXT_COL_KEYS = ['driver', 'team', 'manufacturer', 'class', 'chassis'];
        var SUPER_CARS_HIDDEN_STAT_KEYS = ['top20', 'stage_wins', 'stage_points', 'avg_stage_points', 'laps_led', 'laps_completed_pct', 'pos_diff'];
        var NO_STAGE_STAT_KEYS = ['stage_wins', 'stage_points', 'avg_stage_points'];

        function shouldShowStatsColumn(key) {
          if (seriesKeyStats === 'supercars' && SUPER_CARS_HIDDEN_STAT_KEYS.indexOf(key) >= 0) return false;
          if ((seriesKeyStats === 'arca' || seriesKeyStats === 'nascar_modified') && NO_STAGE_STAT_KEYS.indexOf(key) >= 0) return false;
          return true;
        }

        function makeStatsColumn(key, label, render, extraClass) {
          var cls = STATS_TEXT_COL_KEYS.indexOf(key) >= 0 ? 'col-stat-text' : 'col-stat-num';
          if (key === 'pos' || key === 'car') cls += ' col-num';
          if (extraClass) cls += ' ' + extraClass;
          return { key: key, label: label, render: render, className: cls.trim() };
        }
        ...
        function attachStatsSort(table, columns, dataArray, renderFn, numericKeys) {
          ...
          for (var c = 0; c < ths.length; c++) {
            (function (colIndex) {
              ...
              if (!col || !col.key || col.key === 'pos') return;
        ...
        // F1 uses a dedicated early-return renderer; this flag only affects shared column extras.
        var isF1Stats = (seriesKeyStats === 'f1' || seriesKeyStats.indexOf('f1-') === 0);

        function mapDriverStatsRow(row) {
          return {
            pos: 0,
            car: row.car || '',
            driver: row.driver || '',
        ...
        function mapManufacturerStatsRow(row) {
          return {
            pos: 0,
            manufacturer: row.manufacturer || '',
            races: row.races || 0,
        ...
        function shouldShowManufacturerStats(seriesKey) {
          ...
          return [
            ...
            'gtwce_end', 'gtwce_sprint', 'super_gt'
          ].indexOf(key) >= 0;
        }
        ...
        if (statsClassSelect && hasClassStats) {
          ...
          if (['imsa', 'wec', 'elms', 'gtwce_end', 'gtwce_sprint', 'super_gt'].indexOf(seriesKeyStats) >= 0 && data.classes[0]) {
        ...
        function resolveClassStatsPayload() {
          var sel = statsClassSelect && statsClassSelect.value ? String(statsClassSelect.value) : '';
          if (sel && hasClassStats) {
            for (var i = 0; i < data.classes.length; i++) {
              var cls = data.classes[i] || {};
              if (String(cls.name || '') === sel || String(cls.id || '') === sel) {
                return {
                  rows: Array.isArray(cls.rows) ? cls.rows : [],
                  teams: Array.isArray(cls.teams) ? cls.teams : [],
                  manufacturers: Array.isArray(cls.manufacturers) ? cls.manufacturers : []
                };
              }
            }
          }
          return {
            rows: rows,
            teams: data && data.teams ? data.teams : [],
            manufacturers: data && data.manufacturers ? data.manufacturers : []
          };
        }

        // Mutable datasets so Class filter can refresh Driver / Team / Manufacturer together.
        var statsRows = [];
        var teamData = [];
        var manData = [];
        function refillStatsDatasets() {
          var payload = resolveClassStatsPayload();
          statsRows.length = 0;
          payload.rows.forEach(function (row) { statsRows.push(mapDriverStatsRow(row)); });
          teamData.length = 0;
          payload.teams.forEach(function (row) { teamData.push(mapTeamStatsRow(row)); });
          manData.length = 0;
          payload.manufacturers.forEach(function (row) { manData.push(mapManufacturerStatsRow(row)); });
        }
        refillStatsDatasets();
        ...
        var driverColumns = [
          makeStatsColumn('pos', t('th.pos'), function (row) { return row.pos; })
        ];
        if (hasAnyText(statsRows, 'car')) {
          driverColumns.push(makeStatsColumn('car', t('th.no'), function (row) {
            return formatStatsCarCell(row.car, row.team);
          }, 'col-car'));
        }
        ...
        if (hasAnyText(statsRows, 'team')) {
          driverColumns.push(makeStatsColumn('team', t('th.team'), function (row) {
            return formatStatsTeamCell(row.team, row.car);
          }));
        }
        ...
        if (hasAnyValue(statsRows, 'points')) {
          driverColumns.push(makeStatsColumn('points', t('standings.points'), function (row) { return fmtPoints(row.points); }));
        }
        ...
        if (isStockCarStatsSeries) {
          ...
          if (shouldShowStatsColumn('top20')) {
        ...
        if (hasAnyValue(statsRows, 'dnfs')) {
          driverColumns.push(makeStatsColumn('dnfs', 'DNF', function (row) { return row.dnfs; }));
        }
        if (shouldShowStatsColumn('stage_wins') && (hasAnyValue(statsRows, 'stage_wins') || hasAnyValue(statsRows, 'stage_points'))) {
        ...
        if (shouldShowStatsColumn('laps_led') && hasAnyValue(statsRows, 'laps_led')) {
          driverColumns.push(makeStatsColumn('laps_led', t('stats.laps_led_short'), function (row) { return row.laps_led; }));
        }
        if (shouldShowStatsColumn('laps_completed_pct') && isStockCarStatsSeries && hasAnyValue(statsRows, 'laps_completed_pct')) {
        ...
        if (shouldShowStatsColumn('pos_diff') && hasAnyValue(statsRows, 'avg_start') && hasAnyValue(statsRows, 'pos_diff')) {
        ...
        function renderStatsTable(dataArray) {
          ...
          tbody.innerHTML = filtered.map(function (row, idx) {
            var displayRow = Object.assign({}, row, { pos: idx + 1 });
            var td = driverColumns.map(function (col) {
              return '<td' + (col.className ? ' class="' + col.className + '"' : '') + '>' + col.render(displayRow) + '</td>';
            }).join('');
            return '<tr>' + td + '</tr>';
          }).join('');
        }
        ...
        if (statsTable) {
          ...
          applyDefaultStatsSort(statsRows, seriesKeyStats, driverNumericKeys, 'driver');
          attachStatsSort(statsTable, driverColumns, statsRows, renderStatsTable, driverNumericKeys);
          markStatsSortHeader(statsTable, driverColumns, defaultStatsSortKey(seriesKeyStats, statsRows), -1);
        }
        ...
        // Team stats — dataset comes from refillStatsDatasets() (class-aware).
        ...
        var renderTeamTable = null;
        var teamColumns = null;
        if (teamTable) {
          ...
          if (teamTbody) {
            ...
            renderTeamTable = function (list) {
              var filtered = list.filter(teamPassesFilter);
              teamTbody.innerHTML = filtered.map(function (row, idx) {
                var displayRow = Object.assign({}, row, { pos: idx + 1 });
            ...
            if (hasAnyValue(teamData, 'points')) {
              teamColumns.push(makeStatsColumn('points', t('standings.points'), function (row) { return fmtPoints(row.points); }));
            }
            ...
            if (shouldShowStatsColumn('stage_wins') && (hasAnyValue(teamData, 'stage_wins') || hasAnyValue(teamData, 'stage_points'))) {
            ...
            if (shouldShowStatsColumn('laps_led') && hasAnyValue(teamData, 'laps_led')) {
              teamColumns.push(makeStatsColumn('laps_led', t('stats.laps_led_short'), function (row) { return row.laps_led; }));
            }
            if (shouldShowStatsColumn('laps_completed_pct') && isStockCarStatsSeries && hasAnyValue(teamData, 'laps_completed_pct')) {
            ...
            if (shouldShowStatsColumn('pos_diff') && hasAnyValue(teamData, 'avg_start') && hasAnyValue(teamData, 'pos_diff')) {
            ...
            if (teamHeadRow) {
              ...
              applyDefaultStatsSort(teamData, seriesKeyStats, driverNumericKeys, 'team');
              attachStatsSort(teamTable, teamColumns, teamData, renderTeamTable, driverNumericKeys);
              markStatsSortHeader(teamTable, teamColumns, defaultStatsSortKey(seriesKeyStats, teamData), -1);
            }
        ...
        // Manufacturer stats — dataset from refillStatsDatasets() (class-aware).
        var manTable = document.getElementById('manufacturer-stats-table');
        var manEmpty = document.getElementById('manufacturer-stats-empty');
        ...
        if (!showManufacturerStats) {
          ...
          if (manTbody) {
            ...
            renderManTable = function (list) {
              var filtered = list.filter(manPassesFilter);
              manTbody.innerHTML = filtered.map(function (row, idx) {
                var displayRow = Object.assign({}, row, { pos: idx + 1 });
            ...
            if (hasAnyValue(manData, 'points')) {
              manColumns.push(makeStatsColumn('points', t('standings.points'), function (row) { return fmtPoints(row.points); }));
            }
            ...
            if (shouldShowStatsColumn('stage_wins') && (hasAnyValue(manData, 'stage_wins') || hasAnyValue(manData, 'stage_points'))) {
            ...
            if (shouldShowStatsColumn('laps_led') && hasAnyValue(manData, 'laps_led')) {
              manColumns.push(makeStatsColumn('laps_led', t('standings.laps_led'), function (row) { return row.laps_led; }));
            }
            if (shouldShowStatsColumn('laps_completed_pct') && isStockCarStatsSeries && hasAnyValue(manData, 'laps_completed_pct')) {
            ...
            if (shouldShowStatsColumn('pos_diff') && hasAnyValue(manData, 'avg_start') && hasAnyValue(manData, 'pos_diff')) {
            ...
            if (manHeadRow) {
              ...
              applyDefaultStatsSort(manData, seriesKeyStats, driverNumericKeys, 'manufacturer');
              attachStatsSort(manTable, manColumns, manData, renderManTable, driverNumericKeys);
              markStatsSortHeader(manTable, manColumns, defaultStatsSortKey(seriesKeyStats, manData), -1);
            }
        ...
        if (statsClassSelect) {
          statsClassSelect.addEventListener('change', function () {
            ...
            applyDefaultStatsSort(statsRows, seriesKeyStats, driverNumericKeys, 'driver');
            renderStatsTable(statsRows);
            if (renderTeamTable && teamColumns) {
              if (teamEmpty) teamEmpty.classList.toggle('hidden', teamData.length === 0);
              if (teamData.length > 0) {
                applyDefaultStatsSort(teamData, seriesKeyStats, driverNumericKeys, 'team');
                renderTeamTable(teamData);
              } else if (teamTable && teamTable.querySelector('tbody')) {
                teamTable.querySelector('tbody').innerHTML = '';
              }
            }
            if (showManufacturerStats && renderManTable && manColumns) {
              if (manEmpty) manEmpty.classList.toggle('hidden', manData.length === 0);
              if (manData.length > 0) {
                applyDefaultStatsSort(manData, seriesKeyStats, driverNumericKeys, 'manufacturer');
                renderManTable(manData);
              } else if (manTable && manTable.querySelector('tbody')) {
                manTable.querySelector('tbody').innerHTML = '';
              }
            }
          });
        }
        } // end renderStatsInner
  ...
  API.getSeriesEvents(seriesId, null, { cacheBust: false })
    ...
    .then(function (events) {
      ...
      function renderScheduleRows(list, opts) {
        ...
        // Full Schedule column layouts (A): Circuit + Location always.
        // Multi-race adds Race (session); single-race weekends omit Race.
        var regularBanner = '<tr class="schedule-section-banner"><td colspan="6">' + esc(t('schedule.regular_season')) + '</td></tr>';
        var inSeasonBanner = '<tr class="schedule-section-banner"><td colspan="6">' + esc(t('schedule.in_season_challenge')) + '</td></tr>';
        var playoffsBanner = '<tr class="schedule-section-banner"><td colspan="6">' + esc(t('schedule.playoffs')) + '</td></tr>';
        var cupChaseBanner = '<tr class="schedule-section-banner"><td colspan="6">' + esc(t('schedule.cup_series_chase')) + '</td></tr>';
        var theChaseBanner = '<tr class="schedule-section-banner"><td colspan="6">' + esc(t('schedule.the_chase')) + '</td></tr>';
        ...
        var isPsc = (seriesKeySched === 'psc' || pathSeriesSlug === 'psc');
        var useSfScheduleLayout = isSuperFormula || isPsc;
        // Live F1 seasons with Time column + multi-race grouping: 2024–2026.
        // Older /season/f1-20xx (pre-2024) keep the historical 5-col schedule path.
        var F1_CURRENT_SEASON_SLUG = 'f1-2026';
        var F1_LIVE_SEASON_SLUGS = { 'f1-2024': true, 'f1-2025': true, 'f1-2026': true };
        var isF1CurrentSeasonSlug = (seriesKeySched === F1_CURRENT_SEASON_SLUG || pathSeriesSlug === F1_CURRENT_SEASON_SLUG);
        var isF1ScheduleWithTime = isF1CurrentSeasonSlug
          || !!F1_LIVE_SEASON_SLUGS[seriesKeySched]
          || !!F1_LIVE_SEASON_SLUGS[pathSeriesSlug];
        // Same functional path for bare /series/f1 and /season/f1-2024|2025|2026.
        var isF1 = (seriesKeySched === 'f1' || pathSeriesSlug === 'f1' || isF1ScheduleWithTime);
        // Historical F1 seasons: /season/f1-20xx excluding live 2024/2025/2026 (those use grouped A).
        var isF1Season = !isF1ScheduleWithTime && (seriesKeySched.indexOf('f1-') === 0 || pathSeriesSlug.indexOf('f1-') === 0);
        ...
        if (schedTable) {
          ...
          schedTable.classList.toggle('schedule-table--super-formula', useSfScheduleLayout);
        }
        var unnumberedIds = {
        ...
        if (schedHeadRow) {
          ...
          if (isGroupedRaceSchedule && useSfScheduleLayout) {
            // SF / PSC: Round | Event | Circuit | Location | Date | Time
            // (championship round number in the first column).
            ...
            // A multi-race: Round | Race | Event | Circuit | Location | Date | Time
            schedHeadRow.innerHTML =
              '<th>' + esc(t('th.round')) + '</th>' +
            ...
            // A IMSA: Round | Event | Length | Classes | Circuit | Location | Date
            schedHeadRow.innerHTML =
              '<th>' + esc(t('th.round')) + '</th>' +
              '<th>' + esc(t('th.event')) + '</th>' +
            ...
            // A historical F1: Round | Grand Prix | Circuit | Location | Date (no Time)
            schedHeadRow.innerHTML =
              '<th>' + esc(t('th.round')) + '</th>' +
            ...
            // A single-race: Round | Event | Circuit | Location | Date | Time
            schedHeadRow.innerHTML =
              '<th>' + esc(t('th.round')) + '</th>' +
              '<th>' + esc(t('th.event')) + '</th>' +
              '<th>' + esc(t('th.circuit')) + '</th>' +
            ...
            // A single-race (ELMS, WEC, GTWCE End, Super GT, IndyCar, …)
            schedHeadRow.innerHTML =
              '<th>' + esc(t('th.round')) + '</th>' +
              '<th>' + esc(t('th.event')) + '</th>' +
              '<th>' + esc(t('th.circuit')) + '</th>' +
        ...
        if (window.TGA.normalizeScheduleEvent) {
          list = list.map(function (ev) { return window.TGA.normalizeScheduleEvent(Object.assign({}, ev)); });
        }

        // If no data and no special table — show empty message
        if (!isIndycar && !isSuperFormula && !isPsc && !isF1 && !isF2 && !isF3 && (!list || !list.length)) {
        ...
        if (isF1 && (!list || !list.length)) {
          if (schedBody) {
            var f1StaticYear = (seriesKeySched.match(/^f1-(\d{4})$/) || pathSeriesSlug.match(/^f1-(\d{4})$/) || [])[1] || '2026';
            var f1StaticKey = 'f1_' + f1StaticYear;
            var f1Sched = (window.TGA_STATIC_SCHEDULES &&
              (window.TGA_STATIC_SCHEDULES[f1StaticKey] || window.TGA_STATIC_SCHEDULES.f1)) || [];
            schedBody.innerHTML = f1Sched.map(function (r) {
              var dateParts = String(r.date || '').split(', ');
              var dateOnly = dateParts.length > 1
        ...
        if (isF1Season && (!list || !list.length)) {
          if (schedBody) {
            var f1SeasonYear = (seriesKeySched.match(/^f1-(\d{4})$/) || pathSeriesSlug.match(/^f1-(\d{4})$/) || [])[1] || '2025';
            var f1SeasonSched = (window.TGA_STATIC_SCHEDULES && window.TGA_STATIC_SCHEDULES['f1_' + f1SeasonYear]) || [];
            schedBody.innerHTML = f1SeasonSched.map(function (r) {
              var dateParts = String(r.date || '').split(', ');
              var dateOnly = dateParts.length > 1 ? dateParts.slice(0, -1).join(', ') : (r.date || '—');
              return '<tr>' +
                '<td class="col-num">' + r.rd + '</td>' +
                '<td>' + esc(localizeEventName(r.grand_prix)) + '</td>' +
                '<td>' + esc(localizeVenueLine(r.circuit)) + '</td>' +
        ...
        function eventRow(e, num, opts) {
          ...
          var eventSlugForLink = (isGroupedRaceSchedule && isSupercars && opts.round != null && window.TGA.supercarsWeekendEventSlug)
            ? window.TGA.supercarsWeekendEventSlug(e, opts.round)
            : (e.id || '').toLowerCase().replace(/_/g, '-');
          var link = e.has_detail
            ? '<a href="/event/' + encodeURIComponent(eventSlugForLink) + '" class="event-link">' + esc(eventName) + '</a>'
            : '<span class="event-no-data">' + esc(eventName) + '</span>';
          var dateCell;
          if (isGroupedRaceSchedule) {
            if (opts.groupDateSpan && opts.groupDateSpan > 1) {
              if (opts.groupDateContinuation) {
                dateCell = '';
              } else {
                var groupRangeLabel = date;
                if (opts.groupDateRange && formatDateRange) {
                  var gr = opts.groupDateRange;
                  if (gr.start && gr.end) {
                    groupRangeLabel = formatDateRange(gr.start, gr.end);
                  }
                }
                dateCell = '<td rowspan="' + opts.groupDateSpan + '" class="col-date-span">' + esc(groupRangeLabel || '—') + '</td>';
              }
            } else if (opts.dateContinuation) {
              dateCell = '';
          ...
          if (seriesKeyRow === 'super_formula' || seriesKeyRow === 'psc') {
            // Separate Round column omitted — championship round is the first cell.
            numCell = '';
          }
          var eventCell;
          if (opts.continuation || opts.groupContinuation) {
          ...
          if (isGroupedRaceSchedule) {
            var raceColText = (window.TGA && window.TGA.resolveRaceSessionLabel)
              ...
              : (e._sessionLabel || '');
            if (seriesKeyRow === 'psc' || seriesKeyRow === 'super_formula') {
              // Championship round number (not race-in-weekend index / "Round N" label).
              var champRoundMatch = String(raceColText || e._sessionLabel || '').match(/round\s*(\d+)/i);
              if (champRoundMatch) {
                raceColText = champRoundMatch[1];
              } else {
                var champIdMatch = String(e.id || '').match(/_(\d+)$/);
                raceColText = champIdMatch ? champIdMatch[1] : String(opts.globalRaceNum || opts.round || '');
              }
            } else if (!raceColText && isSupercars && opts.globalRaceNum != null) {
          ...
          // Single-race A helpers: Circuit + geo Location (never put circuit_name in Location).
          var locTextSingleRaw = e.location || '';
          if (isStockCarSeries && window.TGA.stockCarDisplayLocation) {
            locTextSingleRaw = window.TGA.stockCarDisplayLocation(e.circuit_name || '', e.location || '') || '';
          }
          var locTextSingle = locTextSingleRaw ? localizeLocation(locTextSingleRaw) : '—';
          var locationCellSingle;
          if (opts.continuation || opts.groupContinuation) {
            locationCellSingle = '';
          } else if (opts.groupFirst && opts.groupRowSpan && opts.groupRowSpan > 1) {
            locationCellSingle = '<td rowspan="' + opts.groupRowSpan + '" class="col-location-span">' + esc(locTextSingle) + '</td>';
          } else if (opts.circuitContinuation) {
            locationCellSingle = '';
          } else if (opts.circuitFirst && opts.circuitRowSpan && opts.circuitRowSpan > 1) {
            locationCellSingle = '<td rowspan="' + opts.circuitRowSpan + '" class="col-location-span">' + esc(locTextSingle) + '</td>';
          } else {
            locationCellSingle = '<td>' + esc(locTextSingle) + '</td>';
          }
          ...
          if (seriesKeyRow === 'imsa') {
            ...
            var dateLabelImsa = (window.TGA && window.TGA.formatEventRaceStartDate)
              ? window.TGA.formatEventRaceStartDate(e)
              : (formatShortDate ? formatShortDate((e.start_date || e.date || '').slice(0, 10)) : (e.start_date || e.date || ''));
          ...
          if (seriesKeyRow.indexOf('f1-') === 0) {
            // A historical F1: Round | Grand Prix | Circuit | Location | Date
            return (
              '<tr>' +
                numCell +
                eventCell +
          ...
          // A single-race: Round | Event | Circuit | Location | Date | Time
          // (stock-car, IndyCar, PSC, ELMS, WEC, GTWCE End, Super GT, …)
          var timeLabelSingle = getScheduleTimeLabel(e, seriesKeyRow);
          return (
            '<tr' + (opts.continuation ? ' class="schedule-row-continuation"' : '') + '>' +
              ...
              locationCellSingle +
              dateCell +
              '<td class="col-time">' + esc(timeLabelSingle || '—') + '</td>' +
            '</tr>'
          );
        }
        ...
        if (isGroupedRaceSchedule) {
          ...
          for (var idx = 0; idx < list.length;) {
            ...
            round++;
            var roundNum = round;
            if (isF1ScheduleWithTime && isMultiRaceSchedule) {
            ...
            if (size > 1 && !splitWeekendDates) {
              ...
              for (var gd = 0; gd < size; gd++) {
                ...
                if (!gev) continue;
                var gdt = (getRaceStartDateIsoGroup ? getRaceStartDateIsoGroup(gev) : '') ||
                  (gev.start_date || gev.date || '').slice(0, 10);
                if (gdt) groupDates.push(gdt);
              }
              groupDates.sort();
              if (groupDates.length) {
                groupDateStart = groupDates[0];
                groupDateEnd = groupDates[groupDates.length - 1];
              }
            }
            try {
              for (var j = 0; j < size; j++) {
                ...
                var getRaceStartDateIso = window.TGA && window.TGA.getEventRaceStartDateIso;
                var evDate = (getRaceStartDateIso ? getRaceStartDateIso(ev) : '') ||
                  (ev.start_date || ev.date || '').slice(0, 10);
                var dateRowSpan = 1;
                for (var k = j + 1; k < size; k++) {
                  var nextEv = list[start + k];
                  var nextDate = (getRaceStartDateIso ? getRaceStartDateIso(nextEv) : '') ||
                    (nextEv.start_date || nextEv.date || '').slice(0, 10);
                  if (nextDate !== evDate) break;
                  dateRowSpan++;
                }
                var prevDate = j > 0
                  ? ((getRaceStartDateIso ? getRaceStartDateIso(list[start + j - 1]) : '') ||
                    (list[start + j - 1].start_date || list[start + j - 1].date || '').slice(0, 10))
                  : '';
      ...
      if (!Array.isArray(events) || events.length === 0) {
        ...
        if (seriesKeyEvents === 'wec') {
          var wecStatic = [
            ...
            { id: 'WEC_2026_5',        name: 'Lone Star Le Mans',    circuit_name: 'Circuit of the Americas',      location: 'Austin, Texas',    start_date: '2026-09-04', end_date: '2026-09-06' },
...
function renderF1HistoryFromStatic() {
  ...
  for (var season = lastSeason; season >= earliestSeason; season--) {
    ...
    var driverCell = '—';
    if (driver && driver !== '—') {
      driverCell = (window.TGA && window.TGA.driverLinkHtml)
        ? window.TGA.driverLinkHtml(driver)
        : '<a href="/driver/' + encodeURIComponent(slugify(driver)) + '" class="track-link">' + esc(driver) + '</a>';
    }
    rowsHtml += '<tr>' +
      '<td>' + seasonLink + '</td>' +
      '<td>' + races + '</td>' +
...
// ─── Static class/regulations tables (/series/{id}/classes) ───────────────────
function renderImsaClassesSpecIfNeeded() {
  var path = (window.location && window.location.pathname) || '';
  var m = path.match(/^\/series\/([a-z0-9_-]+)\/classes/i);
  if (!m) return;
  var seriesKey = String(m[1] || '').toLowerCase().replace(/-/g, '_');
  var el = document.getElementById('imsa-classes-static');
  var catalog = window.SERIES_CLASSES_SPEC || {};
  var spec = seriesKey === 'imsa' ? window.IMSA_CLASSES_SPEC : catalog[seriesKey];
  if (!el || !spec) return;
...
function renderF1StaticSpecsIfNeeded() {
  ...
  var seasonYear = seasonSpecsMatch ? seasonSpecsMatch[1].slice(3) : '2026';
  var specsTitle = document.getElementById('specs-panel-title');
  if (specsTitle) {
    var techKey = 'specs.tech_regulations_' + seasonYear;
    var techLabel = t(techKey);
    specsTitle.textContent = (techLabel && techLabel !== techKey)
      ? techLabel
      : ('Technical regulations ' + seasonYear);
    specsTitle.classList.remove('hidden');
  }

  var rowsSource = window['F1_' + seasonYear + '_TECH_SPEC']
    || (window.TGA && window.TGA['F1_' + seasonYear + '_TECH_SPEC'])
    || [];
